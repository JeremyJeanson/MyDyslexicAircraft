...
simpleBattery.initialize((battery) => {
  ...
  if (device.screen.height > 300) {
    // SDK5
    const width = batteryString.length * 24;
    _batteryTextContainer.x = (device.screen.width - width - 28) / 2;
  }
...
function updateElevation(activity: simpleActivities.Activity) {
  ...
  if (device.screen.height > 300) {
    ...
    _elevationTextscontainer.x = (device.screen.width - width - 30) / 2;
  }
  else {
    // SDK4
    if (device.screen.height < 300) {
    ...
    else if (device.screen.height === 300) {
