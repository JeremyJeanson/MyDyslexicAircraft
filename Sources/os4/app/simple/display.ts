 ...
 */
export function initialize(onEnteredAOD: () => void, onLeavedAOD: () => void, onDisplayGoOn?: () => void): void {
    ...
    display.addEventListener("change", () => {
        ...
        // Display is on
        if (onDisplayGoOn !== undefined && display.on) onDisplayGoOn();
    });
...
/**
 * Enable AOD if it is available on the current device
 */
export function enableAOD(): void {
    if (aodAvailable) {
...
/**
 * Disable AOD if it is available on the current device
 */
export function disableAOD(): void {
    if (aodAvailable) {
        // tell the system we support AOD
        display.aodAllowed = false;
    }
}